class UserStocksController < ApplicationController
  ...
  def create
    if params[:stock_id].present?
      ...
      stock = Stock.find_by_code(params[:stock_code])
      if stock
        @user_stock = UserStock.new(user: current_user, stock: stock)
      else 
        stock = Stock.new_from_lookup(params[:stock_code])
        if stock.save
          @user_stock = UserStock.new(user: current_user, stock: stock)
        else
          @user_stock = nil
          flash[:error] = "Stock is not valid"
        end
      end
    end
    ...
    respond_to do |format|
      if @user_stock.save
        format.html { redirect_to my_portfolio_path, notice: 
          "Stock #{@user_stock.stock.code} was successfully added" }
